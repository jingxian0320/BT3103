...
Meteor.methods({
  'orderDish': function(recipeName, price, qty) {
    ...
    for (i=0; i<qty; i++){
      OrderDetails.insert({
        ...
        price: price,
        owner_customer: Meteor.userId(),
        shared_customer: [Meteor.userId()],
  ...
  'confirmSplit': function(OrderDetailId, selectedUser) {
    ...
    Meteor.users.update({}, {$pull:{current_order_details: OrderDetailId}}, {multi: true});
