Meteor.methods({
  'viewOrder': function(){
    var user = Meteor.users.findOne(this.userId);
    var orderId = user.current_order;
    var cart = OrderDetails.find({
      order_id: orderId,
      owner_customer: this.userId,
      status:true
    }).fetch();

    return cart
  },

  'splitOrder': function(order_detail_id, user) {
    check(order_detail_id, String);
    user_id = user.user_id;

    var user = Meteor.users.findOne(user_id);
    var orderDetailsList = user.current_order_details;
    var current_order = OrderDetails.findOne(order_detail_id);
    var current_order_sharers = current_order.shared_customer;
    if (current_order_sharers.indexOf(user_id) > -1) {
    	Meteor.users.update({ _id: user_id }, {$pull: { current_order_details: order_detail_id }});
    	OrderDetails.update({_id:order_detail_id}, {$pull: {shared_customer: user_id}});
  	} else {
    	Meteor.users.update({ _id: user_id }, {$addToSet: { current_order_details: order_detail_id }});
    	OrderDetails.update({_id:order_detail_id}, {$addToSet: {shared_customer: user_id}});
    }

  },

  'getOrderDetails': function() {
  	var orderDetailsList = OrderDetails.find({owner_customer: this.userId}).fetch();
  	console.log(orderDetailsList)
  	return orderDetailsList
  },

  'getSharer': function() {
  	var user = Meteor.users.findOne(this.userId);
    // console.log("user" + JSON.stringify(user));
  	var order_id = user.current_order;
    // console.log("order_id" + order_id);
  	var tab = Orders.findOne({_id:order_id}).customers;
    // console.log("tab" + tab);
    var index = tab.indexOf(this.userId);
    if(index != -1) {tab.splice( index, 1 );}
    // console.log("tab" + tab);
    list_sharer = []
    output = {};
    for (i=0;i<tab.length;i++) {
      // console.log("i" + tab[i])
      detail = Meteor.users.findOne(tab[i]);
      email = detail.emails[0].address;
      // console.log(email)
      output.user_id = tab[i];
      output.email = email;
      list_sharer.push(JSON.parse(JSON.stringify(output)));

    }
    // console.log(JSON.stringify(list_sharer))
  	return list_sharer;
  },

  'getCustomerStatus': function(order_detail_id, user) {
  	check(order_detail_id, String);
    user_id = user.user_id;

    var user = Meteor.users.findOne(user_id);
    var orderDetailsList = user.current_order_details;
    var current_order = OrderDetails.findOne(order_detail_id);
    var current_order_sharers = current_order.shared_customer;

    if (current_order_sharers.indexOf(user_id) > -1) {
    	return "share this with you"
  	} else {
    	return ""
    }
  },

  'viewBill': function(){
    var user = Meteor.users.findOne(this.userId);
    var order_details = user.current_order_details;
    var cart = OrderDetails.find({
      _id: { $in: order_details},
      status:true
    }).fetch();
    return cart
  },

  'getBill': function() {

  	var current_orders = Meteor.users.findOne(this.userId).current_order_details;
    console.log("current_orders" + current_orders)
  	var list = [];
    var total = 0;
	  for (i=0;i<current_orders.length;i++) {
		  var dishName = OrderDetails.findOne(current_orders[i]).recipeName;
	    console.log("dishName" + dishName);
	    var dishPrice = OrderDetails.findOne(current_orders[i]).price;
	    console.log("dishPrice" + dishPrice);
	    var dishShare = OrderDetails.findOne(current_orders[i]).shared_customer.length;
	    console.log("dishShare" + dishShare);
	    var finalPrice = +(dishPrice/dishShare).toFixed(2)
	    console.log("finalPrice" + finalPrice);
	    total = total + finalPrice;
	    list.push({name:dishName, price:dishPrice, share:dishShare, finalprice:finalPrice});
	};
	return total
  },

  'getCredits': function() {
  	var credits = Meteor.users.findOne(this.userId).credits;
  	return credits
  },

  'confirmPayment': function(total_amt) {
  	var current_credits = +(Meteor.users.findOne(this.userId).credits).toFixed(2);
    var nets_credits = +(current_credits - total_amt).toFixed(2);
    Meteor.users.update({ _id: this.userId }, {$set: { credits: nets_credits }});
    Meteor.users.update({ _id: this.userId }, {$set: { current_order_details: [] }});
    Meteor.users.update({ _id: this.userId }, {$set: { current_order: '' }});
  }
});

// Initialize bookmark counts. We could use upsert instead.
if (Meteor.isServer) {
  Meteor.startup(function() {
    OrderDetails.remove({})
  });
}
